...
1. Ensure `.env` has local DB secrets (`POSTGRES_PASSWORD`, `POSTGRES_USER`, `POSTGRES_DB`) and Podman is available. `POSTGRES_URL` is derived from `postgres.url` in the env profile unless overridden in TOML.

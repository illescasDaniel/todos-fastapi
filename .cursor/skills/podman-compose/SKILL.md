...
- **Alembic-only bare-metal workflow** — use the `alembic-migrate` skill (`./scripts/migrate.sh`).
...
For bare-metal API dev (venv, `./scripts/migrate.sh`, `./scripts/start.sh`), see Path A in AGENTS.md **Local development paths**.
...
2. **`.env`** at the repo root with:
   ...
   - `POSTGRES_PASSWORD` set (same password as in `DATABASE_URL`)
   - A strong `JWT_SECRET_KEY` (≥ 32 characters, not the template placeholder `change-me-generate-a-secure-random-value`).
...
| `wipe` | `./scripts/wipe.sh` | `down -v` across local infra + app. Requires confirmation unless `--yes` is passed. |
| `seed` | `./scripts/seed.sh` | Reset + migrate + SQL seed via app container (demo users `jane` / `admin`, password `changeme`). |
...
Migrations run on container start when `RUN_MIGRATIONS=true` (default). After `wipe`, run `./scripts/migrate.sh` (and optionally `./scripts/seed.sh` or `up`).

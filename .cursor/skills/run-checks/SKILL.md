---
name: run-checks
description: Run the full quality gate via scripts/run/checks.sh. Local workflow is --fix first, then confirm without --fix. Use after substantive code changes before considering work done.
...
5. Optional: add `--full` on the **first** run only for stack verification after the gate (`verify_stack.sh`; local only, not CI). Still run step 3 afterward.
...
- Individual step scripts live under `scripts/run/`; shared helpers in `scripts/lib.sh` and `scripts/lib/gate.sh`.

...
3. **Destructive MCP tools** (`db_seed`, `db_wipe`, `stack_compose_down` with `remove=true`) require `mcp.allow_destructive=true` in [`config/profiles/local.toml`](config/profiles/local.toml) (or `.cursor/mcp.json` `env`) — see [docs/mcp.md](docs/mcp.md).
...
- `ENV_PROFILE=test` loads [`config/base.toml`](config/base.toml) + [`config/profiles/test.toml`](config/profiles/test.toml); the suite does not depend on `local.toml` or hand-edited `.env`.
...
- Changing Compose, container scripts, Alembic, or `POSTGRES_URL` wiring
...
|------|------|---------------|--------|
| **A — Host app** | Day-to-day API dev (default) | `docker-compose.infra.yml` + `./scripts/start.sh` | `ENV_PROFILE=local` + `profiles/local.toml` |
...
| **C — Production** | Staging / production | `docker-compose.app.base.yml` only via `./scripts/container/deploy.sh` | `ENV_PROFILE=production` + gitignored `profiles/production.toml` |
...
- **Path C lifecycle** (`deploy.sh`, `down.sh --prod`, `logs.sh --prod`): copy [`production.example.toml`](config/profiles/production.example.toml) to `config/profiles/production.toml`, set `ENV_PROFILE=production` — see [docs/deployment.md](docs/deployment.md#path-c--app-only-compose-primary).

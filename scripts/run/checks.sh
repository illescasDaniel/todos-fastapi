...
source "${scripts_dir}/lib/gate.sh"
...
audit_output="$("${scripts_dir}/audit_deps.sh" 2>&1)"
audit_exit=$?
printf '%s\n' "${audit_output}"
if [[ "${audit_exit}" -eq 0 ]]; then
	emit_out="$(printf '%s' "${audit_output}" | "${PYTHON}" "${scripts_dir}/gate_emit.py" audit 2>&1)"
...
if [[ "${FIX}" == true ]]; then
	ruff_output="$("${run_script_dir}/ruff.sh" 2>&1)"
	ruff_exit=$?
	printf '%s\n' "${ruff_output}"
	...
	ruff_check_exit=$?
	printf '%s\n' "${ruff_check_out}"
	emit_out="$(printf '%s\n' "${ruff_check_out}" | "${PYTHON}" "${scripts_dir}/gate_emit.py" ruff-github 2>&1)"
	summary=""
	while IFS= read -r line; do
		if [[ "${line}" == GATE_SUMMARY* ]]; then
			summary="${line}"
...
pyright_json="$("${run_script_dir}/pyright.sh" --outputjson 2>/dev/null)"
pyright_exit=$?
emit_out="$(printf '%s' "${pyright_json}" | "${PYTHON}" "${scripts_dir}/gate_emit.py" pyright 2>&1)"
summary=""
while IFS= read -r line; do
	if [[ "${line}" == GATE_SUMMARY* ]]; then
		summary="${line}"
...
mcp_output="$("${run_script_dir}/mcp_tests.sh" "${forwarded_args[@]}" 2>&1)"
mcp_exit=$?
printf '%s\n' "${mcp_output}"
emit_out="$(printf '%s' "${mcp_output}" | "${PYTHON}" "${scripts_dir}/gate_emit.py" pytest 2>&1)"
summary=""
while IFS= read -r line; do
	if [[ "${line}" == GATE_SUMMARY* ]]; then
		summary="${line}"
...
pytest_output="$("${run_script_dir}/tests.sh" --coverage 2>&1)"
pytest_exit=$?
printf '%s\n' "${pytest_output}"
emit_out="$(printf '%s' "${pytest_output}" | "${PYTHON}" "${scripts_dir}/gate_emit.py" pytest 2>&1)"
summary=""
while IFS= read -r line; do
	if [[ "${line}" == GATE_SUMMARY* ]]; then
		summary="${line}"
...
if [[ "${FULL}" == true ]]; then
	...
	verify_output="$("${scripts_dir}/verify_stack.sh" --skip-coverage 2>&1)"
	verify_exit=$?
	printf '%s\n' "${verify_output}"

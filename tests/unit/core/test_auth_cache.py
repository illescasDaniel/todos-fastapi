...
def bearer_token(settings: Settings) -> str:
	...
	return issuer.issue(user_id=TEST_USER_ID, username="jane", role="user")
...
async def test_get_current_user_uses_cache_on_hit(
	...
	cached = AuthenticatedUser(user_id=TEST_USER_ID, username="cached", role="admin")
	await auth_cache.set_active_user(cached, ttl_seconds=120)
...
async def test_get_current_user_populates_cache_on_miss(
	repo: FakeUserRepository,
	auth_cache: FakeUserAuthCache,
	settings: Settings,
	bearer_token: str,

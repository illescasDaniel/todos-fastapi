...
from todos_app.auth.domain.authorization import ADMIN_ROLE
...
async def test_given_regular_user_creating_todo_when_assigning_owner_then_uses_actor_as_owner(
	...
	# when
	created = await create_todo_for_actor(
		repo,
		entity,
		actor_id=TEST_ACTOR_ID,
		actor_role="user",
...
async def test_given_missing_existing_todo_when_updating_then_refetches_and_persists(
	...
	# when
	updated = await update_todo_for_actor(
		repo,
		TEST_TODO_ID,
		merge,
		actor_id=TEST_ACTOR_ID,
